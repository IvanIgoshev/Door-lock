...
# Motor preferences
steps_moved_forward = 100
steps_moved_backward = 200
speed = 0.003
...
deactivate_all_weekday_passwords = True
...
def door_operating(BeepPin, speed, steps_moved_forward, steps_moved_backward, message, sound, before_door_unlocks_beep):
    if sound == 1:
        GPIO.output(BeepPin, GPIO.HIGH)
    ...
    backward(speed, steps_moved_backward)
    ...
    print('door locking')
    forward(speed, steps_moved_forward)
                    ...
                    if digit.isdigit(): # Next digit entered
                        ...
                        if no_of_digits == 4:
                                ...
                                if (entered_password in passwords):
                                    ...
                                    door_operating(BeepPin, speed, steps_moved_forward,steps_moved_backward, message, sound, before_door_unlocks_beep)
                                ...
                                elif (entered_password == weekday_passwords[int(time.strftime('%w')) - 1]) and active_weekdays[int(time.strftime('%w')) - 1] == True and deactivate_all_weekday_passwords == False :
                                    message = 'password keeper has entered the password'
                                    door_operating(BeepPin, speed, steps_moved_forward, steps_moved_backward, message, sound, before_door_unlocks_beep)
                                    no_of_PK_enters_when_enabled += 1

                                else:
                                    if (entered_password == weekday_passwords[int(time.strftime('%w')) - 1]) and (active_weekdays[int(time.strftime('%w')) - 1] == False or deactivate_all_weekday_passwords == True):
